...
namespace BlazorMock.Components.Pages.Examples.Pokemon.Step0;
...
public partial class ExampleBase : ComponentBase, IDisposable
{
    ...
    {
        var step = await ProgressService.GetStepAsync("pokemon", 0);
    ...
    {
        await ProgressService.MarkStepCompleteAsync("pokemon", 0);
        isComplete = true;
    }

    protected async Task ResetStep()
    {
        await ProgressService.MarkStepIncompleteAsync("pokemon", 0);
        isComplete = false;
    }
