...
namespace BlazorMock.Components.Pages.Examples.Trucking.Step2;
...
public class ExampleBase : ComponentBase, IDisposable
{
    ...
    {
        var step = await ProgressService.GetStepAsync("trucking", 2);
    ...
    {
        await ProgressService.MarkStepCompleteAsync("trucking", 2);
        isComplete = true;
    }

    protected async Task ResetStep()
    {
        await ProgressService.MarkStepIncompleteAsync("trucking", 2);
        isComplete = false;
    }
