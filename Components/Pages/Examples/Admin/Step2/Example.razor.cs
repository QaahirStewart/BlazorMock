...
public partial class ExampleBase : ComponentBase, IDisposable
{
    ...
    {
        var step = await ProgressService.GetStepAsync("admin", 2);
    ...
    {
        await ProgressService.MarkStepCompleteAsync("admin", 2);
        isComplete = true;
    }

    protected async Task ResetStep()
    {
        await ProgressService.MarkStepIncompleteAsync("admin", 2);
        isComplete = false;
    }
