...
namespace BlazorMock.Components.Pages.Examples.AdminDashboard.Step10;
...
public partial class ExampleBase : ComponentBase, IDisposable
{
    ...
    {
        var step = await ProgressService.GetStepAsync("admin-dashboard", 10);
    ...
    {
        await ProgressService.MarkStepCompleteAsync("admin-dashboard", 10);
        isComplete = true;
    }

    protected async Task ResetStep()
    {
        await ProgressService.MarkStepIncompleteAsync("admin-dashboard", 10);
        isComplete = false;
    }
