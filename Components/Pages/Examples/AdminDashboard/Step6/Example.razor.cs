...
namespace BlazorMock.Components.Pages.Examples.AdminDashboard.Step6;
...
public partial class ExampleBase : ComponentBase, IDisposable
{
    ...
    {
        var step = await ProgressService.GetStepAsync("admin-dashboard", 6);
    ...
    {
        await ProgressService.MarkStepCompleteAsync("admin-dashboard", 6);
        isComplete = true;
    }

    protected async Task ResetStep()
    {
        await ProgressService.MarkStepIncompleteAsync("admin-dashboard", 6);
        isComplete = false;
    }
