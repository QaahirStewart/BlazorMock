...
namespace BlazorMock.Components.Pages.Examples.AdminDashboard.Step1;
...
public partial class ExampleBase : ComponentBase, IDisposable
{
    ...
    {
        var step = await ProgressService.GetStepAsync("admin-dashboard", 1);
    ...
    {
        await ProgressService.MarkStepCompleteAsync("admin-dashboard", 1);
        isComplete = true;
    }

    protected async Task ResetStep()
    {
        await ProgressService.MarkStepIncompleteAsync("admin-dashboard", 1);
        isComplete = false;
    }
