...
namespace BlazorMock.Components.Pages.Examples.AdminDashboard.Step12;
...
public partial class ExampleBase : ComponentBase, IDisposable
{
    ...
    {
        var step = await ProgressService.GetStepAsync("admin-dashboard", 12);
    ...
    {
        await ProgressService.MarkStepCompleteAsync("admin-dashboard", 12);
        isComplete = true;
    }

    protected async Task ResetStep()
    {
        await ProgressService.MarkStepIncompleteAsync("admin-dashboard", 12);
        isComplete = false;
    }
