...
namespace BlazorMock.Components.Pages.Examples.AdminDashboard.Step5;
...
public partial class ExampleBase : ComponentBase, IDisposable
{
    ...
    {
        var step = await ProgressService.GetStepAsync("admin-dashboard", 5);
    ...
    {
        await ProgressService.MarkStepCompleteAsync("admin-dashboard", 5);
        isComplete = true;
    }

    protected async Task ResetStep()
    {
        await ProgressService.MarkStepIncompleteAsync("admin-dashboard", 5);
        isComplete = false;
    }
