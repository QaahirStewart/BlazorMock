...
namespace BlazorMock.Components.Pages.Examples.AdminDashboard.Step8;
...
public partial class ExampleBase : ComponentBase, IDisposable
{
    ...
    {
        var step = await ProgressService.GetStepAsync("admin-dashboard", 8);
    ...
    {
        await ProgressService.MarkStepCompleteAsync("admin-dashboard", 8);
        isComplete = true;
    }

    protected async Task ResetStep()
    {
        await ProgressService.MarkStepIncompleteAsync("admin-dashboard", 8);
        isComplete = false;
    }
