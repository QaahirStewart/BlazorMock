...
namespace BlazorMock.Components.Pages.Examples.AdminDashboard.Step3;
...
public partial class ExampleBase : ComponentBase, IDisposable
{
    ...
    {
        var step = await ProgressService.GetStepAsync("admin-dashboard", 3);
    ...
    {
        await ProgressService.MarkStepCompleteAsync("admin-dashboard", 3);
        isComplete = true;
    }

    protected async Task ResetStep()
    {
        await ProgressService.MarkStepIncompleteAsync("admin-dashboard", 3);
        isComplete = false;
    }
