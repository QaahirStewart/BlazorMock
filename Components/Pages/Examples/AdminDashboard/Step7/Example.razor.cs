...
namespace BlazorMock.Components.Pages.Examples.AdminDashboard.Step7;
...
public partial class ExampleBase : ComponentBase, IDisposable
{
    ...
    {
        var step = await ProgressService.GetStepAsync("admin-dashboard", 7);
    ...
    {
        await ProgressService.MarkStepCompleteAsync("admin-dashboard", 7);
        isComplete = true;
    }

    protected async Task ResetStep()
    {
        await ProgressService.MarkStepIncompleteAsync("admin-dashboard", 7);
        isComplete = false;
    }
