...
- Examples of slug generation:
  ...
  - "GetFromJsonAsync<T>" → `#getfromjsonasynct`
  - "@foreach in Razor" → `#foreach-in-razor`
  - "dotnet CLI" → `#dotnet-cli`
  - "Tailwind CSS" → `#tailwind-css`
  - ".NET Asset Pipeline" → `#net-asset-pipeline`

**Creating New Tips**:

> **🚨 CRITICAL REQUIREMENT**: Create ALL tips in TipsService.cs BEFORE linking to them from tutorial steps. Broken tip links severely degrade the learning experience and are considered blocking issues.
...
1. **Add the tip to TipsService.cs** in the appropriate contributor class:
   ...
   - Tooling concepts → Create or use `ToolingTipsContributor`
   - Blazor concepts → Add to existing contributors (`RoutingTipsContributor`, `ServicesTipsContributor`, etc.)
   - Styling concepts → Create or use `StylingTipsContributor`
   - C# concepts → Add to `HttpAndDataTipsContributor` or create `CSharpLanguageTipsContributor`
...
yield return new TipTopic(
    ...
    Category: "Category — Subcategory",  // e.g., "Tooling — .NET", "Blazor — HTTP & Data"
...
3. **Tip categories used**:
   ...
   - `"Tooling — .NET"` - dotnet CLI, MSBuild, etc.
   - `"Tooling — Frontend"` - npm, webpack, etc.
   - `"Tooling — Development"` - Watch mode, hot reload, etc.
   - `"Styling — CSS"` - Tailwind, CSS frameworks
   - `"Blazor — HTTP & Data"` - HttpClient, APIs, JSON
   - `"Blazor — Services"` - DI, service lifetimes
   - `"Blazor — Assets"` - Static files, asset pipeline
   - `"Blazor — Routing"` - Navigation, routes
   - `"Blazor — Forms"` - Input validation, binding
   - `"Blazor — HTML Attributes"` - disabled, checked, selected, etc.
   - `"C# — LINQ"` - Skip, Take, Where, Select, etc.
   - `"C# — Math"` - Math.Ceiling, Math.Floor, etc.
...
5. **MANDATORY verification after adding tips**:
   - ✅ Build the project: `dotnet build -c Release` (must succeed with no errors)
   - ✅ Navigate to `/tips` page and verify the new tip appears in the correct category
   - ✅ Verify the slug matches your link anchor (title "dotnet CLI" → slug `dotnet-cli`)
   - ✅ Click each tip link from the tutorial step and verify it scrolls to the correct section
   - ✅ NO placeholder text - all links must work before considering the step complete
...
**NOTE**: For setup/configuration steps without demo components, rename this section to describe what's being configured (e.g., "🧩 Project Setup Commands" for CLI-based steps).
...
- Code example structure is critical: label → description → bullets → code block
...
- [ ] All code examples have explanatory text and bullets (label → description → bullets → code - NO EXCEPTIONS)
...
### **🚨 CRITICAL: Tip Link Verification (NON-NEGOTIABLE)**
...
- [ ] **Verify tip titles generate correct slugs**:
  - Example: "LINQ Skip/Take" → slug is `linq-skiptake` (lowercase, no special chars, spaces become hyphens)
  - Example: "Math.Ceiling" → slug is `mathceiling` (periods removed)
  - Example: "disabled Attribute" → slug is `disabled-attribute`
